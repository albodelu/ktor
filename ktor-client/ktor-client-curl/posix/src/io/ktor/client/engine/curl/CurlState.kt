...
internal class CurlState(val existingMultiHandle: MultiHandle? = null) {
    ...
    private fun setupUploadContent(easyHandle: EasyHandle, content: ByteArray?) {
        ...
        easyHandle.apply {
            option(CURLOPT_READDATA, stream.asStablePointer())
    ...
    private fun setupMethod(easyHandle: EasyHandle, method: String) = when (method) {
        "GET" -> easyHandle.option(CURLOPT_HTTPGET, 1L)
        "PUT" -> easyHandle.option(CURLOPT_PUT, 1L)
        "POST" -> easyHandle.option(CURLOPT_POST, 1L)
        "HEAD" -> easyHandle.option(CURLOPT_NOBODY, 1L)
        else -> easyHandle.option(CURLOPT_CUSTOMREQUEST, method)
    }
    ...
    fun singleIteration(timeout: Int): List<CurlResponseData> {
        memScoped {
            val transfersRunning = alloc<IntVar>()
            val activeTasks = alloc<IntVar>()
            var repeats = 0

            // See also: curl_multi_wait man page.
            do {
                curl_multi_perform(multiHandle, transfersRunning.ptr).verify()
                curl_multi_wait(multiHandle, null, 0, timeout, activeTasks.ptr).verify()
