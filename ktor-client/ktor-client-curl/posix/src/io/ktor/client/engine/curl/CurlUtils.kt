...
internal fun CURLMcode.verify() {
    if (this != CURLM_OK) throw CurlIllegalStateException("Unexpected curl verify: ${curl_multi_strerror(this)?.toKString()}")
}

internal fun CURLcode.verify() {
    if (this != CURLE_OK) throw CurlIllegalStateException("Unexpected curl verify: ${curl_easy_strerror(this)?.toKString()}")
}
